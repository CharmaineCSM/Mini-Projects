...
def battle():
    ...
    if choice == "1":
        p1.attack
        current_enemy.attack_character()
        input(">")
    elif choice == "2":
        p1.heal()
        input(">")
    ...
    if p1.hp <= 0:
        clear()
        print(f"{current_enemy.name} has deafeated {p1.name}....")
        fight = False
        play = False
        run = False
        print("GAME OVER")
    ...
    while play:
        ...
        if not standing:
            if biom[map[y][x]]["title"] == "MOUNTAINS":
                ...
                time.sleep(3)
                print(
                    "Your breath coming in ragged gasps as you gaze out at the world spread beneath you."
                )
                time.sleep(3)
                print(
                    "Before you lies a small, weathered treasure box, nestled amidst the rocks and snow."
                )
                time.sleep(3)
                ...
                if last_level == "1":
                    print(
                        "You have unlocked the treasure box... You look inside and see something moving... something ALIVE..."
                    )
                    time.sleep(3)
                    clear()
                    print(")/_ \n<' \ \n/)  ) \n---/'-" "---")
